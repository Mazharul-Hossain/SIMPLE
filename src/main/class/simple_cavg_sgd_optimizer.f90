...
type :: cavg_sgd_diagnostics
    ...
    real     :: effective_support_sum = 0.0
...
type :: cavg_sgd_optimizer
    ...
    real                    :: eta0       = 0.2
    real                    :: min_support = 3.0
    real                    :: max_rel_step = 0.25
...
contains
    ...
    subroutine cavg_sgd_diag_reset( self )
        ...
        self%effective_support_sum = 0.0
    ...
    subroutine cavg_sgd_diag_record_preserved( self, low_support, trust_rejected, support, effective_support,&
        &proposed_rel_step )
        class(cavg_sgd_diagnostics), intent(inout) :: self
        logical, optional, intent(in) :: low_support, trust_rejected
        real,    optional, intent(in) :: support, effective_support, proposed_rel_step
        self%n_preserved = self%n_preserved + 1
        if( present(low_support) )then
            if( low_support ) self%n_low_support = self%n_low_support + 1
        endif
        if( present(trust_rejected) )then
            if( trust_rejected ) self%n_trust_rejected = self%n_trust_rejected + 1
        endif
        if( present(support) )then
            self%support_sum = self%support_sum + support
            self%support_min = min(self%support_min, support)
            self%support_max = max(self%support_max, support)
        endif
        if( present(effective_support) )then
            self%effective_support_sum = self%effective_support_sum + effective_support
            self%effective_support_min = min(self%effective_support_min, effective_support)
            self%effective_support_max = max(self%effective_support_max, effective_support)
        endif
        if( present(proposed_rel_step) )&
            &self%proposed_rel_step_max = max(self%proposed_rel_step_max, proposed_rel_step)
    ...
    subroutine new( self, params, which_iter )
        ...
        self%min_support = params%sgd_cavg_min_support
        self%max_rel_step = params%sgd_cavg_max_rel_step
    ...
    subroutine preconditioned_real3_inplace( self, old_cavg, rho, stats, diag, support, throw_on_nonfinite,&
        ...
        real,                       optional, intent(in)    :: effective_support
        ...
        n_nonfinite = 0
        support_val = 0.0
        effective_support_val = 0.0
        if( present(support) ) support_val = support
        if( present(effective_support) ) effective_support_val = effective_support
        if( present(accepted) ) accepted = .false.
        l_throw = .false.
        if( present(throw_on_nonfinite) ) l_throw = throw_on_nonfinite
        allocate(steps(size(stats,1),size(stats,2),size(stats,3)), source=0.0)
        do k = 1, size(stats,3)
            do j = 1, size(stats,2)
                do i = 1, size(stats,1)
                    rho_ijk = rho(i,j,k)
            ...
            enddo
        enddo
        proposed_rel_step = 0.0
        if( old_sq > DTINY )then
            proposed_rel_step = real(sqrt(step_sq / old_sq))
        ...
        if( n_nonfinite > 0 )then
            ...
            if( present(diag) )then
                ...
                call diag%record_preserved(support=support_val, effective_support=effective_support_val,&
                    &proposed_rel_step=proposed_rel_step)
            endif
            if( l_throw ) THROW_HARD('CAVG SGD preconditioned real update produced nonfinite values')
            return
        endif
        ...
        do k = 1, size(stats,3)
            do j = 1, size(stats,2)
                do i = 1, size(stats,1)
                    rho_ijk = rho(i,j,k)
                    if( rho_ijk > TINY )then
                        stats(i,j,k) = rho_ijk * (old_cavg(i,j,k) + steps(i,j,k))
                    else
    ...
    subroutine preconditioned_complex3_inplace( self, old_cavg, rho, stats, diag, support, throw_on_nonfinite,&
        ...
        complex(kind=c_float_complex), allocatable :: steps(:,:,:)
        eta_t = self%eta()
        grad_sq = 0.0_dp
        step_sq = 0.0_dp
        old_sq  = 0.0_dp
        ...
        l_throw = .false.
        if( present(throw_on_nonfinite) ) l_throw = throw_on_nonfinite
        allocate(steps(size(stats,1),size(stats,2),size(stats,3)), source=cmplx(0.0,0.0,kind=c_float_complex))
        do k = 1, size(stats,3)
            do j = 1, size(stats,2)
                do i = 1, size(stats,1)
                    rho_ijk = rho(i,j,k)
        ...
        if( n_nonfinite > 0 )then
            ...
            if( l_throw ) THROW_HARD('CAVG SGD preconditioned complex update produced nonfinite values')
            return
        endif
        if( proposed_rel_step > self%max_rel_step )then
            deallocate(steps)
    ...
    subroutine write_update_diag( self, label, diag )
        ...
        real :: support_min, support_mean, support_max, effective_support_min
        ...
        support_mean = 0.0
        support_max  = 0.0
        n_evaluated = diag%n_updated + diag%n_preserved
        if( n_evaluated > 0 )then
            support_min  = diag%support_min
            support_mean = diag%support_sum / real(n_evaluated)
            support_max  = diag%support_max
        endif
        effective_support_min = 0.0
        if( n_evaluated > 0 ) effective_support_min = diag%effective_support_min
    ...
    subroutine kill( self )
        ...
        self%which_iter = 0
        self%eta0       = 0.2
        self%min_support = 3.0
        self%max_rel_step = 0.25
    ...
    subroutine update_diag( diag, support, effective_support, proposed_rel_step, grad_sq, step_sq, old_sq,&
        ...
        diag%effective_support_sum = diag%effective_support_sum + effective_support
