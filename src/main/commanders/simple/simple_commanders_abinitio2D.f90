...
contains
    ...
    subroutine exec_abinitio2D( self, cline )
        ...
        if( start_stage < 1 .or. start_stage > nstages ) THROW_HARD('invalid joint2D checkpoint start stage')
        if( stop_stage < start_stage .or. stop_stage > nstages ) THROW_HARD('invalid joint2D checkpoint stop stage')
        if( start_stage > 1 .and. checkpoint_last_iter < 1 )&
            &THROW_HARD('joint2D checkpoint continuation requires the last completed iteration')
        allocate(stage_parms(nstages))
        ! read project
        call spproj%read(params%projfile)
        call set_dims                   ! set downscaling
        if( start_stage == 1 ) call inirefs ! deal with initial references only for a fresh run
      ...
      contains
        ...
        subroutine execute_terminal_prob_pass
            ...
            write(logfhandle,'(A)')&
                &'>>> ABINITIO2D SGD STAGE: stage=terminal refine=prob prob_assign=likelihood'//&
