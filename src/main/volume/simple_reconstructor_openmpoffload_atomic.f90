...
contains
    ...
    subroutine insert_slices_batch(sz, cdim, cdim2D, clb2D, clb, fpllims, nsym,&
        ...
        integer :: win(3,2), nyqsq, iwinsz, h, i, k, l, m, isym, iy, iz, ky, mz, x1, x2, x3
        ...
        !$omp& loc,w_ctfsq,h_sq,k_max_h,k_lo,k_hi,w_comp,x1,x2,x3)
        do i = 1, sz
            do isym = 1, nsym
                        ...
                        do k = k_lo, k_hi
                            ...
                            do iz = 1, WDIM
                                ...
                                do iy = 1, WDIM
                                    ...
                                    if( even(i) )then
                                        !$omp atomic update
                                        cmatexp_e(x1, ky, mz) = cmatexp_e(x1, ky, mz) + wx(1) * w_comp
                                        !$omp atomic update
                                        cmatexp_e(x2, ky, mz) = cmatexp_e(x2, ky, mz) + wx(2) * w_comp
                                        !$omp atomic update
                                        cmatexp_e(x3, ky, mz) = cmatexp_e(x3, ky, mz) + wx(3) * w_comp

                                        !$omp atomic update
                                        rhoexp_e(x1, ky, mz) = rhoexp_e(x1, ky, mz) + wx(1) * w_ctfsq
                                        !$omp atomic update
                                        rhoexp_e(x2, ky, mz) = rhoexp_e(x2, ky, mz) + wx(2) * w_ctfsq
                                        !$omp atomic update
                                        rhoexp_e(x3, ky, mz) = rhoexp_e(x3, ky, mz) + wx(3) * w_ctfsq
                                    else
                                        !$omp atomic update
                                        cmatexp_o(x1, ky, mz) = cmatexp_o(x1, ky, mz) + wx(1) * w_comp
                                        !$omp atomic update
                                        cmatexp_o(x2, ky, mz) = cmatexp_o(x2, ky, mz) + wx(2) * w_comp
                                        !$omp atomic update
                                        cmatexp_o(x3, ky, mz) = cmatexp_o(x3, ky, mz) + wx(3) * w_comp
                                        
                                        !$omp atomic update
                                        rhoexp_o(x1, ky, mz) = rhoexp_o(x1, ky, mz) + wx(1) * w_ctfsq
                                        !$omp atomic update
                                        rhoexp_o(x2, ky, mz) = rhoexp_o(x2, ky, mz) + wx(2) * w_ctfsq
                                        !$omp atomic update
                                        rhoexp_o(x3, ky, mz) = rhoexp_o(x3, ky, mz) + wx(3) * w_ctfsq
                                    endif
                                end do
                            end do
                        end do
